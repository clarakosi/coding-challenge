...
import { getReferrals, addReferral, deleteReferral, updateReferral } from '../Actions'
  ...
  render() {
    ...
    return (
      <div>
        ...
        <input type="text" name="newReferral" value={this.state.newReferral} onChange={this.newReferralChangeHandler} />
          ...
          <tbody>
            {this.props.referrals.map((referral, index) => {
              return<tr key={referral.id}>
                ...
                <td onClick={() => {this.deleteReferral(referral.id)}}>Delete</td>
              </tr>
            })}              
          </tbody>
        </table>
        <form onSubmit={this.updateReferral} style={{ display: this.state.update ? null: 'none' }} >
          <input name="title" value={this.state.title} onChange={this.updateChangeHandler} />
...
export default connect(mapStateToProps, { getReferrals, addReferral, deleteReferral, updateReferral })(HomePage)